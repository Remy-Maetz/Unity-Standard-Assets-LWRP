...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAIControl : MonoBehaviour
    {
        ...
        [SerializeField] private bool m_Driving = false;                                                  // whether the AI is currently actively driving or stopped.
        [SerializeField] private Transform m_Target = null;                                              // 'target' the target object to aim for.
        [SerializeField] private bool m_StopWhenTargetReached = false;                                    // should we stop driving when we reach the target?
        [SerializeField] private float m_ReachTargetThreshold = 2;                                // proximity to target to consider we 'reached' it, and stop driving.

        private float m_RandomPerlin = 0f;             // A random value for the car to base its wander on (so that AI cars don't all wander in the same pattern)
        private CarController m_CarController;    // Reference to actual car controller we are controlling
        private float m_AvoidOtherCarTime = 0f;        // time until which to avoid the car we recently collided with
        private float m_AvoidOtherCarSlowdown = 0f;    // how much to slow down due to colliding with another car, whilst avoiding
        private float m_AvoidPathOffset = 0f;          // direction (-1 or 1) in which to offset path to avoid other car, whilst avoiding
